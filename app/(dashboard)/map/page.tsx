...
export default function MapPage() {
    ...
    async function fetchParcels() {
      try {
        ...
        if (res.ok) {
          ...
          setParcels(data.parcels ?? data.results ?? data ?? [])
        }
      } catch {
        // API not available yet — that's expected during build
      } finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
        ...
        popupRef.current = new maplibregl.Popup({
          ...
          .setHTML(
            `<div style="font-family: system-ui, sans-serif; color: #e5e7eb;">
              <div style="font-weight: 600; font-size: 14px; margin-bottom: 6px; color: #f9fafb;">
                ${props.name}
              </div>
      ...
      map.on('mouseleave', 'parcel-circles', () => {
    ...
    <div className="relative h-[calc(100vh-4rem)] -mx-4 -mt-6 sm:-mx-6 lg:-mx-8">
      ...
      <div className="absolute left-4 top-4 z-10 flex items-center gap-3 rounded-xl bg-gray-900/90 px-4 py-3 shadow-xl backdrop-blur-sm border border-gray-700/50">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-xs font-medium text-gray-200 w-7 text-right tabular-nums">
            {minScore}
          </span>
        </div>
      ...
      <div className="absolute bottom-6 right-4 z-10 rounded-xl bg-gray-900/90 px-4 py-3 shadow-xl backdrop-blur-sm border border-gray-700/50">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-300">80+ High</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="inline-block h-3 w-3 rounded-full bg-amber-500" />
            <span className="text-xs text-gray-300">60-79 Medium</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="inline-block h-3 w-3 rounded-full bg-red-500" />
            <span className="text-xs text-gray-300">&lt;60 Low</span>
          </div>
        </div>
      </div>
